...
pub type Type = llhttp_type;
pub type Error = llhttp_errno;
pub type Method = llhttp_method;
pub type LenientFlags = llhttp_lenient_flags;

pub type Callback = llhttp_cb;
pub type DataCallback = llhttp_data_cb;

#[derive(Copy, Clone, Debug, Default)]
pub struct Settings(llhttp_settings_t);
...
macro_rules! cb_wrapper {
  ($fname:ident, $func:ident, $data_type:ty) => {
    #[inline]
    unsafe extern "C" fn $fname(arg1: *mut llhttp::llhttp_t) -> libc::c_int {
...
impl Settings {
  pub fn new() -> Settings {
    ...
    unsafe {
      llhttp_settings_init(&mut settings.0);
    }
    settings
  }
}
...
impl<'a, T> Parser<'a, T> {
  ...
  pub fn init(&mut self, settings: &Settings, lltype: Type) {
    unsafe {
      llhttp_init(&mut self._llhttp, lltype.into(), &settings.0);
    }
  }

  #[inline]
  pub fn parse(&mut self, data: &[u8]) -> Error {
    unsafe { llhttp_execute(&mut self._llhttp, data.as_ptr() as *const c_char, data.len()) }
  }

  #[inline]
  pub fn finish(&mut self) -> Error {
    unsafe { llhttp_finish(&mut self._llhttp) }
  }

  #[inline]
  pub fn message_needs_eof(&self) -> bool {
    unsafe {
      match llhttp_message_needs_eof(&self._llhttp) {
        1 => true,
        _ => false,
      }
    }
  ...
  pub fn should_keep_alive(&self) -> bool {
    unsafe {
      match llhttp_should_keep_alive(&self._llhttp) {
        1 => true,
        _ => false,
      }
    }
  ...
  pub fn pause(&mut self) {
    unsafe { llhttp_pause(&mut self._llhttp) }
  }

  #[inline]
  pub fn resume(&mut self) {
    unsafe { llhttp_resume(&mut self._llhttp) }
  }

  #[inline]
  pub fn resume_after_upgrade(&mut self) {
    unsafe { llhttp_resume_after_upgrade(&mut self._llhttp) }
  }

  #[inline]
  pub fn errno(&self) -> Error {
    unsafe { llhttp_get_errno(&self._llhttp) }
  }

  #[inline]
  pub fn get_error_reason(&self) -> &CStr {
    unsafe { CStr::from_ptr(llhttp_get_error_reason(&self._llhttp)) }
  }

  #[inline]
  pub fn get_error_pos(&self) -> &CStr {
    unsafe { CStr::from_ptr(llhttp_get_error_pos(&self._llhttp)) }
  }
  ...
  pub fn method(&self) -> Method {
    llhttp_method(self._llhttp.method as c_uint)
  }
  ...
  pub fn lltype(&self) -> Type {
    llhttp_type_t(self._llhttp.type_ as c_uint)
  }

  #[inline]
  pub fn reset(&mut self) {
    unsafe { llhttp_reset(&self._llhttp as *const _ as *mut _) }
  }
