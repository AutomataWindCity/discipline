...
export const enum SerializeErrorType {
  Stringify,
}

export type SerializeError = TUnique<"JsonSerializeError", {
  readonly type: SerializeErrorType.Stringify,
  readonly error: unknown,
}>;

export const SerializeError_Stringify = (error: unknown): SerializeError => {
  return Unique.create({
    type: SerializeErrorType.Stringify,
    error,
  });
};

export type Serializer<T> = TUnique<"JsonSerializer", {
  readonly __jsonable: (me: T) => Jsonable,
  readonly serialize: (me: T) => null;
}>;
