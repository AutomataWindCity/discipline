...
fn write_add_rule(
  ...
  code.write(" ");
  write_compound_value_with_writer(
    code, 
    &collection.schema, 
...
impl<'a> crate::x::procedures::rule_group_rules::TransactionWriter for Adapter<'a> {
  fn add_rule(
    &self, 
  ...
  fn delete_rule(
    ...
    write_delete_rule(transaction.code(), self.collection, rule_id);
  }
  ...
  fn update_rule(
    ...
    let mut draft = CompoundValueWriteDestinationForUpdate::new();

    WriteUpdates::write_updates(
      original, 
      modified, 
      &self.collection.schema.rule, 
      &mut draft,
    );
