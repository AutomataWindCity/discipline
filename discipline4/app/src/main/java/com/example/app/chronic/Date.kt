...
value class Date private constructor(private val timestamp: Long) {
    ...
    companion object {
        ...
        fun now(): Date = fromLocalDate(LocalDate.now(ZoneOffset.UTC))
    ...
    fun getDayStart(): Date = this
    
    fun getDurationTillMidnight(): Duration = Duration.fromMillisecondsOrThrow(
        toLocalDate().plusDays(1).atStartOfDay(ZoneOffset.UTC).toInstant().toEpochMilli() - timestamp
    )
    
    fun isLaterThan(other: Date): Boolean = timestamp > other.timestamp
    fun isEarlierThan(other: Date): Boolean = timestamp < other.timestamp
    fun isAt(other: Date): Boolean = timestamp == other.timestamp
    
    override fun toString(): String = toLocalDate().toString()
    
    operator fun plus(days: Long): Date = Date(timestamp + days * Duration.MILLISECONDS_PER_DAY)
    operator fun minus(days: Long): Date = Date(timestamp - days * Duration.MILLISECONDS_PER_DAY)
    operator fun compareTo(other: Date): Int = timestamp.compareTo(other.timestamp)
}