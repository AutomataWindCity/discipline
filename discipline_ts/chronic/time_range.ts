import { Duration, Time, Tried, TextualError, Unique } from "../x.ts";
...
export type TimeRange = Unique<typeof BRAND, "TimeRange", RawTimeRange>;

export const construct = (from: number, till: number): TimeRange => {
  return { from, till } satisfies RawTimeRange as TimeRange;
};
 ...
 */
export const isIntraday = (it: TimeRange): boolean => {
  return it.till <= MAXIMUM_FROM_TIMESTAMP;
};

/**
 * Spanning across two days
 */
export const isInterday = (it: TimeRange): boolean => {
  return it.till > MAXIMUM_FROM_TIMESTAMP;
};
