...
export type Time = Unique<typeof BRAND, "Time", number>;

const construct = (timestamp: number): Time => {
...
export const fromTimestampOrError = (
  ...
  if (!Number.isInteger(timestamp)) {
    TextualError.changeContext(textualError, "Creating a Time from a millisecond timestamp since midnight");
